...
function SectionHeading({ eyebrow, title, href, locale }: { eyebrow: string; title: string; href: "/shop"; locale: ReturnType<typeof getLocale> }) { return <div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-eyebrow text-muted-foreground">{eyebrow}</p><h2 className="mt-2 text-h3 text-foreground sm:text-2xl">{title}</h2></div><Link to={href} search={{ locale }} className="inline-flex items-center gap-2 text-small font-medium text-foreground">View all <ArrowRight className="size-4" /></Link></div>; }