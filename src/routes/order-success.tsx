...
function OrderSuccessPage() { const { locale, order: orderNumber } = Route.useSearch(); const t = getTranslations(locale); if (!orderNumber) return <div dir={localeDirections[locale]} lang={locale} className="min-h-screen bg-background"><SiteHeader locale={locale} t={t} /><main className="px-6 py-24 text-center text-muted-foreground">We could not locate this confirmation.</main><SiteFooter t={t} /></div>; return <OrderSuccessContent locale={locale} orderNumber={orderNumber} />; }
function OrderSuccessContent({ locale, orderNumber }: { locale: ReturnType<typeof getLocale>; orderNumber: string }) { const { data: order } = useSuspenseQuery(queryOptions({ queryKey: ["order-confirmation", orderNumber], queryFn: () => getOrderConfirmation({ data: { orderNumber } }) })); const t = getTranslations(locale); return <div dir={localeDirections[locale]} lang={locale} className="min-h-screen bg-background"><SiteHeader locale={locale} t={t} />{order ? <ConfirmationView {...order} locale={locale} /> : <main className="px-6 py-24 text-center text-muted-foreground">We could not locate this confirmation.</main>}<SiteFooter t={t} /></div>; }
