...
function FoundationPage() { const locale = platformConfig.market.defaultLanguage; const t = getTranslations(locale); return <div className="min-h-screen bg-background"><SiteHeader locale={locale} t={t} /><main><EmptyState title="Track order">Order tracking will be available after order operations launch.<div className="mt-7"><Button asChild variant="outline"><Link to="/">{t.shell.back}</Link></Button></div></EmptyState></main><SiteFooter t={t} /></div>; }
